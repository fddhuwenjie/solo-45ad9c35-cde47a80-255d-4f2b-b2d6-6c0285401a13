...
常温扭矩、液压张拉或超声复核合格只代表装配温度下合格；升温后螺栓、夹持件
（法兰叠层）与垫片的热膨胀不同步，螺母约束下载荷重新分配，可能出现**螺栓
过载、垫片压溃或密封面分离**。在紧固 API 之外增加**热态预紧力校核**：
工况与修订全部写入 SQLite，求解器为纯函数（`app/thermal.py`）。

`POST /procedures/{id}/thermal-cases`（工艺须 approved 及以后）冻结：

| 字段 | 说明 |
|---|---|
| `source_type` / `source_id` | 逐栓初始载荷来源：**已确认**超声批次或**已确认**张拉方案（缺省取最新） |
| `bolt` / `members[]` | 螺栓与各夹持件长度、截面、弹性模量 E、热膨胀系数 α、材料物性温度区间 |
| `gasket` | 垫片有效承压面积、厚度、α、物性区间与**压缩—回弹曲线**（折点：压缩量/加载压力/回弹压力） |
| `bolt_zones` | 逐栓温度分区（按栓号 1..N 顺序） |
| `reference` | 装配参考温度（螺栓/夹持件/垫片各自基准温度） |
| `limits` | 螺栓允许载荷、最小密封面压、垫片压溃面压、相邻温度节点最大间隔 |
| `nodes[]` | **带时标**的分区温度节点（时刻升序，每节点给出各分区三部件温度） |

各物理量在部件/折点级声明单位（长度 mm/cm/m/in，面积 mm²/cm²/m²/in²，
模量/压力 MPa/GPa/Pa/psi/ksi）；同类量声明不一致即**单位冲突**证据缺口。

**求解器**按温度节点建立单栓轴向变形协调与平衡：

```
(F−F0)·(1/k_b + 1/k_c) + (x_g(F) − x_g0) = D
D = Σ α_i L_i ΔT_m + α_g L_g ΔT_g − α_b L_b ΔT_b
```

垫片沿压缩—回弹双折线工作：升温沿加载支，卸载从历史峰值按回弹支形状平移
（升温—降温循环路径相关，峰值逐节点推进）。逐栓输出每个节点的**热态载荷、
垫片面压、密封裕量（面压−最小密封面压）与所在支**，并给出**首次越限时刻**
（接触分离/压溃/螺栓超载/密封裕量不足）。

**证据缺口（版本照常落库 201，但不可确认，列栓号与对应时间区间）**：

| reason | 含义 |
|---|---|
| `initial_load_missing` | 已确认来源中缺该栓逐栓初始载荷 |
| `temperature_gap` | 相邻节点间隔超冻结上限，或节点缺该栓所属分区温度（含受影响栓号/区间） |
| `material_curve_coverage` | 部件温度超出冻结材料物性区间，禁止外推 |
| `gasket_curve_coverage` | 所需面压/压缩超出压缩或回弹曲线折点（峰值恰处上限折点的数值微扰容差内仍可解） |
| `unit_conflict` | 同类物理量声明单位不一致（整案只列缺口、不产出逐时结果） |
| `non_convergence` | 某节点协调方程迭代不收敛 |

出现**接触分离、垫片压溃、螺栓超载或密封裕量不足**同样不可确认（逐节点
状态 + 首次越限时刻）。`POST /thermal-cases/{id}/confirm` 在无缺口、无越限时
才可通过（记录确认人与意见）。

**修订**：人工采用替代边界或材料/垫片曲线，须 `POST /thermal-cases/{id}/revisions`
**写明理由**并至少变更一项冻结内容，派生新修订（revision+1、parent 链接，
旧修订废止不覆盖）；空修订拒绝。`GET /thermal-cases/{id}/diff` 给出冻结参数、
温度时序、初载来源与逐栓初载变化。修订查询、确认结果与 `GET /procedures/{id}/package`
的 `thermal` 字段返回**所选版本**的冻结参数、逐时结果与人工决定。
...
```
app/
  schemas.py     Pydantic 输入核验（工艺 + 超声 + 对中预检 + 轨迹 + 现场约束 + 液压张拉 + 热态校核）
...
tests/test_thermal.py    热态协调求解、滞回路径、接触分离/压溃/超载/密封裕量、缺口与修订
samples/                 请求样例
```
